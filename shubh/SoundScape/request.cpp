...
void getPlaylists(QString accessToken)
{
    ...
    QString authBearer = "Authorization: Bearer " + accessToken;
    headers = curl_slist_append(headers, authBearer.toStdString().c_str());
    std::cout << authBearer.toStdString().c_str();
...
void initRequest(QString authCode)
{
    ...
    QString myAuthCode = authCode;
    QString redirectURI = "http%3A%2F%2Flocalhost%3A8080%2Fcallback";
    QString clientId = "f6d6bc1e9dce452eb812a9f9973925ee";
    QString clientSecret = "8971b907959143adaf59cd19f8ae04ce";
    QString postFields = "grant_type=authorization_code&code=" + myAuthCode + "&redirect_uri=" + redirectURI + "&client_id=" + clientId + "&client_secret=" + clientSecret;
    //string response;
    curl_easy_setopt(curl_handle, CURLOPT_WRITEDATA, (void*)&chunk);
    curl_easy_setopt(curl_handle, CURLOPT_POSTFIELDS, postFields.toStdString().c_str());
